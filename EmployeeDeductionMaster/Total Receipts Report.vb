...
Public Class Total_Receipts_Report

    Dim totalamount As Decimal

    Private Sub loadReceiptsPerCustomer(ByVal sdate As Date, ByVal edate As Date)
        DataGridView1.DataSource = Nothing
        Dim qry As String = "select PayerID,ModeName,SUM(Amount) from Receipts inner join ModeOfPayment on Receipts.ModeID = ModeOfPayment.ModeID where Receipts.Status not in ('New','Batch','Banking','Declined') and TransactionDate between '" & sdate & "' and '" & edate & "' group by PayerID,ModeName order by PayerID"
        ...
        Catch ex As Exception
            If con.State = ConnectionState.Open Then
                con.Close()
            End If
            MessageBox.Show(ex.Message, "loadReceiptsPerCustomer()", MessageBoxButtons.OK, MessageBoxIcon.Error)
            MessageBox.Show(ex.StackTrace, "loadReceiptsPerCustomer()", MessageBoxButtons.OK, MessageBoxIcon.Error)
        Finally
            con.Close()
        End Try
    End Sub
    ...
    Private Sub loadReceiptsPerCustomerAndPaymentMode(ByVal sdate As Date, ByVal edate As Date, ByVal ModeID As Integer)
        ...
        Dim qry As String = "select PayerID,ModeName,SUM(Amount) from Receipts inner join ModeOfPayment on Receipts.ModeID = ModeOfPayment.ModeID where Receipts.Status not in ('New','Batch','Banking','Declined') and Receipts.ModeID = '" & ModeID & "' and TransactionDate between '" & sdate & "' and '" & edate & "' group by PayerID,ModeName order by PayerID"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        loadReceiptsPerCustomer(DateTimePicker1.Value, DateTimePicker2.Value)
        ...
        Next
        Label3.Text = getCurrency(totalamount)
