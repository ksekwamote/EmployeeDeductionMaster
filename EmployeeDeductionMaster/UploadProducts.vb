        ...
        If decision("Are you sure you want to proceed?") = DialogResult.Yes Then
            If DataGridView1.Rows.Count > 0 Then
                For Each row As DataGridViewRow In DataGridView1.Rows
                    saveProduct(row.Cells(0).Value.ToString, Integer.Parse(row.Cells(1).Value.ToString), row.Cells(2).Value.ToString, Integer.Parse(row.Cells(3).Value.ToString), Integer.Parse(row.Cells(4).Value.ToString), row.Cells(5).Value.ToString, row.Cells(6).Value.ToString, row.Cells(7).Value.ToString, row.Cells(8).Value.ToString, row.Cells(10).Value.ToString)
                Next
