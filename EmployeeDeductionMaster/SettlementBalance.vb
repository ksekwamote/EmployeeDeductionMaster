...
Public Class SettlementBalance
    ...
    Public datestring, towhom, re, thisserves, ifyouintend, thitoaccount, pleaseuse, formoreinfo, yours, supervisor, title, email, pleasebeinformed, pleaseensure, product, productbalance, totalbalance, instalmentdescription, totalinstalments As String
    ...
    Public rs As RequestSettlement
    ...
    Public Sub generateSettlementReport()
        ...
        If dscustomer.Tables(0).Rows.Count > 0 Then
            ...
            For Each row As DataGridViewRow In rs.DataGridView2.Rows
                product = row.Cells(0).Value
                productbalance = ": " + getCurrency(Decimal.Parse(row.Cells(1).Value))
                instalmentdescription = ": " + getCurrency(Decimal.Parse(row.Cells(2).Value)) + " monthly instalment(s)"
                dtsettlement.Rows.Add(datestring, towhom, re, thisserves, ifyouintend, thitoaccount, pleaseuse, formoreinfo, yours, supervisor, title, email, pleasebeinformed, pleaseensure, product, productbalance, totalbalance, instalmentdescription, totalinstalments)
            Next
            rs.Close()
    ...
    Private Sub loadProducts(ByVal CustomerID As String)
        ...
        Dim qry As String = "select AutoID,Products,HasSystem,'0' as SettlementBalance,'0' as Instalment,HasSystem as Include,'0' as LedgerBalance,'' as Comment1,'' as Comment2,'' as LastDateOfDeduction from Products where UseInSettlement = 'Yes' and AutoID in (select ProductID from EmployerProducts where Status = 'Active' and EmployerID in (select EmployerID from CustomerDetails where CustomerID = '" & CustomerID & "'))"
